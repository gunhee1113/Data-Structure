...
public class SortingTest
{
	...
	// 최대 힙을 만드는 함수
	private static void buildMaxHeap(int[] value) { // Chatgpt : 힙 정렬 구현 과정에서 함께 구현
	...
	// 최대 힙 유지 함수
	private static void maxHeapify(int[] value, int i, int heapSize) { // Chatgpt : 힙 정렬 구현 과정에서 함께 구현
		int left = 2 * i + 1;
		int right = 2 * i + 2;
		int largest = i;
	...
	private static void swap(int[] value, int i, int j) { // Chatgpt : 힙 정렬 구현 과정에서 함께 구현
		int temp = value[i];
		value[i] = value[j];
		value[j] = temp;
	}
	...
	// 두 배열을 합치는 함수
	private static void switching_merge(int left, int mid, int right, int[] A, int[] B){ //2021-중간고사 5번 문제 아이디어 참고하여 코드 작성
		int i = left;
		int j = mid+1;
		int t = left;
		while (i <= mid && j <= right) {
	...
	private static void quickSort(int[] value, int left, int right) { // Chatgpt : 퀵 정렬 구현 과정에서 함께 구현
		if (left < right) {
	...
	private static void radixSort(int[] arr, int length) { // Chatgpt : radix sort 구현 과정에서 함께 구현
	...
	private static int getMax(int[] arr, int length) { // Chatgpt : radix sort 구현 과정에서 함께 구현
		int max = arr[0];
	...
	private static void countSort(int[] arr, int length, int[] count, int exp) { // Chatgpt : radix sort 구현 과정에서 함께 구현
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
    private static char DoSearch(int[] value) // Chatgpt : docx에 있는 search 알고리즘에 대한 아이디어를 구현 후 수정
	{
		...
		return 'Q';// 조건이 충족되지 않을 경우, 기본적으로 Merge Sort 추천
	}


	private static int getMaxDigit(int[] value){ // Chatgpt : search 알고리즘 구현 과정에서 함께 구현
	...
	private static int getMin(int[] arr, int length) { // Chatgpt : 위 getMax 참고하여 수정
		int min = arr[0];
		for (int i = 1; i < length; i++) {
			if (arr[i] < min) {
				min = arr[i];
	...
	private static double getCollisionRate(int[] arr) { // Chatgpt : search 알고리즘 구현 과정에서 함께 구현
	...
	private static double getSortedRate(int[] arr) { // Chatgpt : search 알고리즘 구현 과정에서 함께 구현
